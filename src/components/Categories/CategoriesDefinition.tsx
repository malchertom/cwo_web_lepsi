...
export const teamOpen = (
    <motion.div
        ...
        exit={{ opacity: 0 }}
        className="categories-definition">
        <p>Kategorie CWO Team OPEN je určena pro čtyřčlenné týmy ve variabilním složení:</p>
        <ul>
            <li>4 ženy</li>
            <li>3 ženy + 1 muž</li>
            <li>2 ženy + 2 muži</li>
        ...
        <p>Tyto kombinace mohou být napříč všemi hmotnostními i věkovými skupinami. Výsledné pořadí všech týmů v rámci kategorie Team Open je určeno seřazením hodnot Sinclairových bodů týmů. Sinclairovy body celého týmu jsou výsledkem součtů všech SB každého člena daného týmu (u žen je násobeno koefientem 1,5).</p>
    </motion.div>
)
...
export const teamOpenDead = (
    <motion.div
        ...
        exit={{ opacity: 0 }}
        className="categories-definition">
        <p>Kategorie CWO Team OPEN (+1 deadlift) je určena pro jakýkoliv tým z kategorie Team Open. jehož členové se rozhodnou změřit síly navíc také v soutěži mrtvého tahu a nejen v klasickém vzpírání.</p><p>Každý tým této kategorie má možnost doplnit svou čtveřici o jednoho dalšího hostujícího člena, závodícího pouze v soutěži mrtvého tahu. V případě, že se daný tým rozhodne soutěžit v kategorii CWO Team OPEN (+1 deadlift) bez možnosti doplnit tým o hostujícího siláka, může libovolně zvolit jednoho závodníka/závodnici ze svého týmu, který/á se soutěže v mrtvém tahu zúčastní za svůj tým. Tyto kombinace mohou být napříč všemi hmotnostními i věkovými skupinami.</p><p>Výsledné pořadí všech týmů v soutěži je určeno seřazením součtu bodů, který tvoří aritmetický průměr Sinclairových bodů čtyř vzpěračů (u žen je počet Sinclairových bodů násoben koefientem 1,5) a Wilksových bodů jednoho atleta za deadlift.</p>
    </motion.div>
)